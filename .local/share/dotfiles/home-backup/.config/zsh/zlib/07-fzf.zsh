...
export FZF_DEFAULT_COMMAND="
        fd --type f --hidden --follow --exclude .git --ignore-file .gitignore \
...
#export FZF_PREVIEW_COMMAND="--preview 'opener {}'"
#export FZF_DEFAULT_COMMAND='fd . --hidden --follow --type file' \
#fzf --bind 'ctrl-d:reload(fd . --hidden --follow --type directory),ctrl-f:reload($FZF_DEFAULT_COMMAND)'


# binary -> display indication of this
# directory -> use `$DIR_PREVIEW_COMMAND`
# files -> `bat` is installed on all my machines
#export FZF_CTRL_T_OPTS='--preview='"'"' [[ $(file --mime {}) =~ binary ]] && echo {} is a binary file || [[ $(file --mime {}) =~ directory ]] && '"$DIR_PREVIEW_COMMAND"' {} || bat --style=numbers --color=always {}'"'"''
